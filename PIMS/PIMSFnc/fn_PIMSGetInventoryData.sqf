...
_this spawn {
	...
	if (!isNull _player) then {
		...
		[missionNamespace, [format ["PIMS_InventoryName_%1", _playerUid], _inventoryName]] remoteExec ["setVariable", _player];
		[missionNamespace, [format ["PIMS_InventoryItems_%1", _playerUid], _items]] remoteExec ["setVariable", _player];
		[missionNamespace, [format ["PIMS_InventoryMoney_%1", _playerUid], _inventoryMoney]] remoteExec ["setVariable", _player];
		[missionNamespace, [format ["PIMS_InventoryDataReady_%1", _playerUid], true]] remoteExec ["setVariable", _player];
	} else {
